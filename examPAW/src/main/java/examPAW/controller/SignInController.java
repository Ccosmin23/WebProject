...
//    private SessionService sessionService;
    ...
    @PostMapping("/test")
    public ResponseEntity<?> test(@RequestBody Map<String, String> loginData) {
        String text = loginData.get("text");

        System.out.println("\n text = " + text);

        try {
            return ResponseEntity.ok().body("{\"message\":\"request trimis cu succes\"}");

        } catch (UsernameNotFoundException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{\"message\":\"nu s-a gasit ceea ce cautati\"}");
        }
    }
